...
Image::Image(Image&& rhs) noexcept
    : image_(std::exchange(rhs.image_, nullptr)),
      ...
      mipLevels_(std::exchange(rhs.mipLevels_, 1)),
...
Image createImageFromData(Device& device, const ImageData& data) {
    ...
    vk::DeviceSize imageSize = data.width * data.height *
                               (data.numChannels == 3 ? 4 : data.numChannels);
    ...
    if (data.numChannels == 3) {
        ...
        std::ranges::copy(
            pixels, static_cast<glm::u8vec4*>(stagingBuffer.getMappedMemory())
        );
    } else {
        auto pixels = std::span{
            data.pixels,
            static_cast<size_t>(data.width * data.height * data.numChannels)
        };
        std::ranges::copy(
            pixels, static_cast<unsigned char*>(stagingBuffer.getMappedMemory())
        );
    }
    ...
    uint32_t mipLevels =
        static_cast<uint32_t>(
            std::floor(std::log2(std::max(data.width, data.height)))
        ) +
        1;
