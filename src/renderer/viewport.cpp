...
namespace yuubi {
    ...
    Viewport::Viewport(std::shared_ptr<vk::raii::SurfaceKHR> surface, std::shared_ptr<Device> device) :
    ...
    Viewport& Viewport::operator=(Viewport&& rhs) {
        if (this != &rhs) {
            ...
            std::swap(normalImage_, rhs.normalImage_);
            std::swap(normalImageView_, rhs.normalImageView_);
            std::swap(normalImageFormat_, rhs.normalImageFormat_);
            std::swap(aoImage_, rhs.aoImage_);
            std::swap(aoImageView_, rhs.aoImageView_);
            std::swap(aoImageFormat_, rhs.aoImageFormat_);
        }
        return *this;
    }
    ...
    void Viewport::createSwapChain() {
        ...
        const auto capabilities = device_->getPhysicalDevice().getSurfaceCapabilitiesKHR(**surface_);
    ...
    vk::SurfaceFormatKHR Viewport::chooseSwapSurfaceFormat() const {
        const auto availableFormats = device_->getPhysicalDevice().getSurfaceFormatsKHR(**surface_);
    ...
    vk::PresentModeKHR Viewport::chooseSwapPresentMode() const {
        const auto availablePresentModes = device_->getPhysicalDevice().getSurfacePresentModesKHR(**surface_);
